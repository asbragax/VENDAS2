...
include_once("components/apagar/action/listar_apagar_mes.php");
...
// include_once('components/pessoa/action/listar_consultor.php'); 
include_once("components/caixa/view/consulta/modal.php");
 ?>
<div class="panel panel-inverse">
    <div class="panel-heading">
        <h3 class="panel-title">
            Total de contas em aberto: R$ <?php echo number_format(array_sum(array_column($listaApagar2, "valor"))/100, 2, ",", "."); ?>
        </h3>
    </div>
...
<div class="panel panel-inverse">
    <div class="panel-heading">
        <h4 class="panel-title">Lista de contas a pagar em aberto</h4>
            ...
            <table id="table-consApagars" data-page-length='20' class="table table-bordered table-hover table-striped w-100">
                ...
                <tbody>
                    <?php for ( $i = 0; $i < count($listaApagar2); $i++ ) {
                        $arquivo_nota = explode(".", $listaApagar2[$i]['arquivo_nota']);
                        $arquivo_boleto = explode(".", $listaApagar2[$i]['arquivo_boleto']);
                        ?>

                        <tr>
                            <td>
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input pags" name="pags[]" id="check_<?php echo $listaApagar2[$i]['id_parcela']."_".$listaApagar2[$i]["valor"]; ?>" value="<?php echo $listaApagar2[$i]['id_parcela']; ?>">
                                    <label class="form-check-label" for="check_<?php echo $listaApagar2[$i]['id_parcela']."_".$listaApagar2[$i]["valor"]; ?>"></label>
                                </div>
                            </td>
                            <td><?php echo $listaApagar2[$i]["vencimentof"]; ?></td>
                            <td><?php echo $listaApagar2[$i]["nome"]; ?></td>
                            <td><?php echo $listaApagar2[$i]["nome_fornecedor"]; ?></td>
                            <td><?php echo ($listaApagar2[$i]["num"]+1)." de ".$listaApagar2[$i]["prestacao"]; ?></td>
                            <td>R$ <?php echo number_format($listaApagar2[$i]["valor"]/100, 2, ",", "."); ?></td>
                            <td class="none">
                                ...
                                <td class="none">
                                    <?php if($listaApagar2[$i]['status'] == 0 && $listaApagar2[$i]['forma_pag'] == 2){ ?>
                                    <a class="btn btn-success btn-xs btn-icon" href="?pagApagar=<?php echo $listaApagar2[$i]['id_parcela']; ?>" data-bs-toggle="tooltip" data-placement="auto" title data-original-title="Pagar">
...
<a href="#" class="float-custom bg-primary-300" id="btnpag">
    <i class="my-float" id="num_pags"></i>
</a>
...
<script>
const TOTALAPAGAR = <?php echo safe_json_encode(array_sum(array_column($listaApagar2, "valor"))); ?>;
</script>
